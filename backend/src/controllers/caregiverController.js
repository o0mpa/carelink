...
export const updateCaregiverProfile = async (req, res) => {
    try {
        ...
        const allowedFields = [
            ...
            'education_docs', 'certificates', 'criminal_record', 'references', 'skills',
            'day_rate_a', 'day_rate_b', 'day_rate_c', 'day_rate_d','profile_picture'];
        //filter updates
        ...
        await db.promise().query(
            `UPDATE caregiver_profiles SET ${setClause} WHERE user_id = ?`, [...values, id]
        );
        res.status(200).json({message: 'Caregiver Profile Updated Successfully'});
    } catch (error) {
        res.status(500).json({error: error.message});
    }
}
...
export const uploadCaregiverPicture = [
    upload.single('picture'),
    async (req, res) => {
        try {
            const {id} = req.user;
            await db.promise().query(`UPDATE caregiver_profiles SET profile_picture = ? WHERE user_id = ?`, [req.file.path, id]);
...
//change password
export const changeCaregiverPassword = async (req, res) => {
    try {
        const {id} = req.user;
