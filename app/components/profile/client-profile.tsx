...
const defaultUser: ProfileUser = {
  ...
  medical: { bloodType: "", allergies: "Not provided", facility: "" }, 
  //  doctor: "",
  careNeeds: { skills: [], specialties: [] },
  emergency: {
...
useEffect(() => {
  const fetchProfile = async () => {
    try {
      ...
      localStorage.setItem("carelink_profile", JSON.stringify(p));
      setUser(mapProfile(p as Record<string, unknown>));
    } catch {
      // fallback to localStorage if offline
      const raw = localStorage.getItem("carelink_profile");
      if (raw) {
        try { setUser(mapProfile(JSON.parse(raw))); } catch { /* */ }
      }
    }
  };
  fetchProfile();
...
const handlePictureUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const token = localStorage.getItem("carelink_token") ?? "";
  try {
    const res = await fetch("http://localhost:5000/api/client/upload-picture", {
      method: "POST",
    ...
    if (!res.ok) {
      ...
      // re-fetch profile to show new picture
  ...
  const handleLogout = async () => {
    try {
      ...
      // If request fails, still log out client-side
    } finally {
      clearAuth();
      navigate("/login");
    }
              ...
              <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="grid gap-6 md:grid-cols-2">
                  ...
                  <div className="rounded-xl border-2 border-blue-100 bg-blue-50/50 p-4">
                    ...
                    <p className="mt-1 text-sm text-gray-900"><span className="font-semibold">Doctor's Name/Facility:</span> {user.medical.facility}</p>
                  </div>
                </div>
              </div>
            )}
