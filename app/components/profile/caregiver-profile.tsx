  ...
  return {
    ...
    documents: [
      ...
      { label: "National ID",                    path: String(profile.national_id     || "") || null },
          // ? String(profile.national_id).replace(/\\/g, '/') : "") || null },
...
const DEFAULT: CaregiverViewData = {
  name: "Your Name",
  title: "Certified Caregiver",
  rating: 0,
  email: "",
...
export default function CaregiverProfile() {
  ...
  // Phase 1: paint instantly from localStorage
  const initialData = useMemo(() => {
    const saved = getProfile();
    if (!saved || typeof saved !== "object") return DEFAULT;
    return mapProfile(saved as Record<string, unknown>);
  }, []);

  const [caregiver, setCaregiver] = useState<CaregiverViewData>(initialData);

  // Phase 2: background fetch → refresh UI + keep localStorage in sync
  useEffect(() => {
...
const handleLogout = async () => {
  try {
    ...
    // If the request fails, still log out client-side
  } finally {
    clearAuth();
    navigate("/login");
  }
      ...
      <main className="container mx-auto px-4 py-4 sm:px-6 md:px-10">
        <div className="mx-auto max-w-5xl">
          <Link
            ...
            className="inline-flex items-center gap-2 text-sm py-3 font-semibold text-emerald-500 transition-colors hover:text-emerald-900 hover:underline"
          >
            ← Back to Dashboard
          </Link>
          ...
          <div className="mb-6 flex flex-col items-center justify-between rounded-3xl bg-white/90 p-8 shadow-lg backdrop-blur-md ring-2 ring-blue-100 sm:flex-row sm:p-10">
            ...
            const file = e.target.files?.[0];
              ...
              const token = localStorage.getItem("carelink_token") ?? "";
              try {
            const res = await fetch("http://localhost:5000/api/caregiver/upload-picture", {
            method: "POST",
              ...
              <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="mb-10 grid gap-6 md:grid-cols-2">
                  <div className="rounded-xl border-2 border-gray-100 bg-gray-50 p-4">
                    <p className="text-xs font-bold uppercase tracking-wider text-gray-500">Contact Details</p>
                  ...
                  <div className="rounded-xl border-2 border-gray-100 bg-gray-50 p-4">
                    <p className="text-xs font-bold uppercase tracking-wider text-gray-500">Service Area</p>
                    <p className="mt-2 text-sm text-gray-900">{caregiver.serviceArea || "—"}</p>
                  </div>
                </div>

                <section>
                  <h2 className="mb-1 pb-3 text-2xl font-bold text-emerald-900">Accepted Salaries Per Day</h2>
                  <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
                    {caregiver.salaries.map((item) => (
                      <div key={item.label} className="rounded-2xl border-2 border-gray-100 bg-white p-2 text-center shadow-sm">
                        <p className="mb-2 text-[10px] font-bold uppercase text-gray-700">{item.label}</p>
              ...
              <div className="animate-in fade-in slide-in-from-bottom-2 space-y-12 duration-300">
                <section>
                  ...
                  <div className="rounded-2xl border-2 border-gray-100 bg-gray-50/50 p-6 sm:p-8">
                    ...
                    <div className="border-t border-gray-200 pt-3">
                      <p className="mb-2 text-xs font-bold uppercase tracking-wider text-gray-400">Medical Specialties</p>
                      <p className="text-md font-medium italic text-slate-700">"{caregiver.specialties}"</p>
                    </div>
                  </div>
                </section>
                ...
                <section>
                  ...
                  {caregiver.documents.length === 0 ? (
                    <p className="text-sm text-gray-400">Documents will appear here once your profile is loaded.</p>
                  ) : (
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                      {caregiver.documents.map((doc) => (
                        <div key={doc.label} className="flex items-center justify-between rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                          <div className="flex flex-col">
                            <span className="text-[12px] font-bold uppercase text-gray-700">{doc.label}</span>
                            {doc.path ? (
                              <a href={`http://localhost:5000/${doc.path}`} target="_blank" rel="noopener noreferrer" className="text-xs font-semibold text-emerald-600 hover:underline">
                                View File
                              </a>
                            ) : (
                              <span className="text-xs font-semibold text-gray-400">Not uploaded</span>
                            )}
                          </div>
                          {doc.path ? (
                            <a href={`http://localhost:5000/${doc.path}`} target="_blank" rel="noopener noreferrer" className="text-gray-400 transition-colors hover:text-emerald-600">
